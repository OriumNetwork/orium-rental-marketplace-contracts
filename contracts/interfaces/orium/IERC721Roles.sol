...
interface IERC721Roles {
    event RoleGranted(bytes32 indexed _role, address indexed _account, uint256 indexed _tokenId, uint64 _expirationDate, bytes _data);
    event RoleRevoked(bytes32 indexed _role, address indexed _account, uint256 indexed _tokenId);

    function grantRole(bytes32 _role, address _account, uint256 _tokenId, uint64 _expirationDate, bytes calldata _data) external;
